  ...
  <div id="header" class="new_header">
    <div class="hd_bar">
        <div class="bd_bar_bd cle">
            <ul class="welcome">
                <li id="ECS_MEMBERZONE" v-if="name">
                <!-- <router-link :to="'/app/home/member/userinfo'">{{name}}</router-link> -->
                &nbsp;[
            ...
            <ul id="userinfo-bar">
                <li class="more-menu" @mouseover="isShowVip=true" @mouseout="isShowVip=false"> <a>用户</a><i class="iconfont arrow"> </i>
                    <div class="more-bd" :class="{show:isShowVip}">
                        <div class="list">
                        <router-link :to="'/app/home/member/order'">我的订单</router-link>
                        <router-link :to="'/app/home/member/userinfo'">用户信息</router-link>
    ...
    <div class="hd_main cle">
        ...
        <router-link to="/app/home/index" class="lizi_logo">
            <img src="../../static/images/head/logo.gif" alt="商城">
        </router-link>
        ...
        <div class="hd_nav_bd cle">
                        ...
                        <ul>
                            <li class="first" v-for="(item,index) in allMenuLabel" @mouseover="overChildrenmenu(index)" @mouseout="outChildrenmenu(index)">
                              <h3 style="background:url(../../static/images/head/1449088788518670880.png) 20px center no-repeat;">
                                <router-link :to="'/app/home/list/'+item.parent">{{item.name}}</router-link> </h3>
                                <div class="J_subCata" id="J_subCata" v-show="showChildrenMenu ===index"  style=" left: 215px; top: 0px;">
                                    <div class="J_subView" >
                                      <div v-for="list in item.subCategory">
                                        <dl>
                                          <dt>
                                            <router-link :to="'/app/home/list/'+list.parent">{{list.name}}</router-link>
                                          </dt>

                                          <dd>
                                            <router-link  v-for="childrenList in list.subCategory" :key="childrenList.parent" :to="'/app/home/list/'+childrenList.parent">{{childrenList.name}}</router-link>
                                          </dd>
                                        </dl>
                                        <div class="clear"></div>
                                      </div>
            ...
            <ul class="sub_nav cle" id="sub_nav">
                ...
                <template v-for="(item,index) in allMenuLabel">
                  <li>
                    <div v-if="item.tab" >
                      <router-link :to="'/app/home/list/'+item.parent" >{{item.name}}</router-link>
                    </div>
                  </li>
                </template>
            ...
            <div class="hd_cart" id="ECS_CARTINFO"  @mouseover="overShopCar" @mouseout="outShopCar">
             <router-link class="tit" :to="'/app/shoppingcart/cart'" target = _blank>
                    ...
                    <b class="iconfont">&#xe600;</b>去购物车结算<span><i class="iconfont">&#xe645;</i></span></router-link>
                    <!-- <em class="num" id="hd_cartnum" style="visibility: visible;">{{goods_list.total}}</em></router-link>
                        <div class="list" v-show="showShopCar">
                            <div class="data">
                               <dl v-for="(item,index) in goods_list.goods_list">
                                <dt><router-link :to="'/app/home/productDetail/'+item.id" target = _blank><img :src="item.goodsFrontImage"></router-link></dt>
                                <dd>
                                  <h4><router-link :to="'/app/home/productDetail/'+item.id" target = _blank>{{item.name}}</router-link></h4>
                                  <p><span class="red">{{item.goods_price}}</span>&nbsp;<i>X</i>&nbsp;{{item.nums}}</p>
                                  <a title="删除" class="iconfont del" @click="deleteGoods(index,item.id)">×</a></dd>
                              </dl>
                            </div>
                            <div class="count">共<span class="red" id="hd_cart_count">{{goods_list.list.total}}</span>件商品哦~
...
import { getBrands, getCategorys ,deleteShopCart } from  '../../api/api'
export default {
    data(){
        return {
            hotSearch:[],//热词
    ...
    methods:{
        ...
        getMenu(){//获取菜单
          getCategorys({
            params:{}
                    ...
                    this.allMenuLabel = response.data.data.list
                    console.log("allMenLabel",this.allMenuLabel)
                })
                .catch(function (error) {
                  console.log(error);
                });
        ...
        getBrand(){//获取热搜
            getBrands()
                .then((response)=> {
                    this.brands= response.data.data.list
    ...
    created(){
        console.log("getListgetList",this.goods_list)
