                        ...
                        <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
                            <tbody>
                                ...
                                <tr v-for="item in orders">
                                    <td align="center" bgcolor="#ffffff"><a class="f6" @click="goDetail(item.orderSn)">{{item.orderSn}}</a></td>
                                    <td align="center" bgcolor="#ffffff">{{formatDate(item.addTime)}}</td>
                                    <td align="center" bgcolor="#ffffff">￥{{item.orderMount}}元</td>
                                    <!-- <td v-if="item.status == 'paying' " align="center" bgcolor="#ffffff">待支付</td>
                                    <td v-if="item.status == '' " align="center" bgcolor="#ffffff">待支付</td>
                                    <td v-if="item.status == 'TRADE_SUCCESS' " align="center" bgcolor="#ffffff">已支付</td>
                                    <td v-if="item.status == 'TRADE_CLOSED' " align="center" bgcolor="#ffffff">已关闭</td> -->
                                   <td align="center" bgcolor="#ffffff"><font class="f6"><a @click="cancelOrder(item.orderSn)">取消订单</a></font></td>
                                </tr>
                            </tbody>
                        </table>
        ...
        methods: {
            ...
            getOrder () {
                getOrders().then((response)=> {
                    console.log("订单",response.data.data.list)
            ...
            cancelOrder (id) {
                ...
                delOrder(this.delete).then((response)=> {
                  alert('订单删除成功')
                }).catch(function (error) {
                    console.log(error);
                });
