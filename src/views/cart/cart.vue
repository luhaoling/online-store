      ...
      <div class="goods-list">
        <ul>
          <li class="cle hover" style="border-bottom-style: none;" v-for="(item,index) in goods.goods_list" :key="item.goods_id">
            ...
            <div class="pic">
              <a target="_blank"> <img :alt="item.name" :src="item.goodsFrontImage"></a>
            </div>
            <div class="name">
            ...
            <div class="price-xj"><span></span>
              <em id="total_items_3137">￥{{(item.shopPrice * item.nums).toFixed(2)}}元</em>
            </div>
            <div class="del">
              <a class="btn-del" @click="deleteGoods(index, item.id)">删除</a>
            </div>
          </li>
        </ul>
      </div>
...
<script>
  import {getShopCarts,clearShopCart, createOrder, updateShopCart, deleteShopCart, getAddress} from '../../api/api'
  export default {
    data () {
      return {
        receiveData: {},
    ...
    created () {
      ...
      getShopCarts().then((response)=> {
        ...
        this.goods.goods_list = response.data.data.list;
        console.log("goods_list",this.goods.goods_list)
        response.data.data.list.forEach(function(entry) {
          totalPrice += entry.shopPrice*entry.nums;
        });
    ...
    methods: {
      ...
      updateCart(id,nums, checked,index) {
        this.update.id=id;
        this.update.goods=this.goods.goods_list[index].goods;
        this.update.nums=this.goods.goods_list[index].nums+1;
        updateShopCart(this.update
        ).then((response)=> {
            this.goods.goods_list[index].nums = this.goods.goods_list[index].nums + 1;
          // 更新store数据
          this.$store.dispatch('setShopList');
      ...
      chooseGoods(index,obj) {
        ...
        if(!this.goods.goods_list[index].checked) {
          for (var i=arr.length-1;i>=0;i--){
            if (this.arr[i].goodsId==this.goods.goods_list[index].goods){
              this.arr.splice(i,1)
            }
          }
          this.totalPrice = this.totalPrice-this.goods.goods_list[index].nums*this.goods.goods_list[index].shopPrice
        } else {
          console.log("add")
          console.log("element",this.arr)
          ...
          goodEle.goodsId=this.goods.goods_list[index].goods
          goodEle.num=this.goods.goods_list[index].nums
          this.arr.push(goodEle)
          console.log("after add",this.arr)
      ...
      setTotalPrice(){
        ...
        for(var i = 0;i<goods_list.length;i++){
          if(goods_list[i].checked) {
            ...
            totalPrice=(totalPrice+goods_list[i].nums* goods_list[i].shopPrice).toFixed(2);
          }
        }
      ...
      deleteGoods(index,id) { //移除购物车
        alert('您确定把该商品移除购物车吗');
        this.delete.id=id;
        deleteShopCart(this.delete).then((response)=> {
      ...
      reduceCartNum(index, id) { //删除数量
        if(this.goods.goods_list[index].nums<=1){
          ...
          this.update.goods=this.goods.goods_list[index].goods;
          this.update.nums=this.goods.goods_list[index].nums-1;
          updateShopCart(this.update).then((response)=> {
            this.goods.goods_list[index].nums = this.goods.goods_list[index].nums - 1;
            // 更新store数据
            this.$store.dispatch('setShopList');
            //更新总价
      ...
      balanceCount () { // 结算
          ...
          this.createOrder.sellInfo.goodsInvInfo=this.arr
          this.createOrder.orderMount=this.totalPrice
          this.createOrder.address=this.receiveData.address
          this.createOrder.singerMobile=this.receiveData.singerMobile
          this.createOrder.signerName=this.receiveData.signerName
          console.log("reqreq",this.createOrder.sellInfo.goodsInvInfo)
          console.log("reqreq,arr",this.arr)
