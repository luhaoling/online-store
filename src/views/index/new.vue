<template>
    <div>
        ...
        <div class="newopro-l fl">
          <a href=""><img src="http://shop.projectsedu.com/static/images/xinpin.0abdd07.jpg" width="224" height="478"></a>
        </div>
        <div class="newopro-r fr">
            ...
            <ul class="newgoods_fastbuy">
                <li class="prolist-cent clearfix have_num" v-for="item in newopro">
                    ...
                    <img :src="item.goodsFrontImage" style="height: 158px;width: 158px;" class="zom" :alt="item.name">
                    </a></router-link>
                    </div>
                    <div class="prolist-r fl">
                        ...
                        <p><em class="c333"></em>{{item.goodsBrief}}</p><div>
                        <span class="p-price"><em class="fastbuy_price">￥{{item.shopPrice}}元</em><del>原价:￥{{item.shopPrice}}元</del></span>
                        <a href="" class="p-buy fr ibg">立即抢购</a>
                       <span class="p-time fr">销量：{{item.soldNum}}件</span>
                    </div>
                    </div>
                </li>
            </ul>
        ...
        getOpro(){
          getNew()
            .then((response)=> {
               ...
               console.log("getNew",response.data.data.list)
                this.newopro = response.data.data.list
            })
