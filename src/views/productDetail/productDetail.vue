            ...
            <div class="detail_wrap">
                <div class="detail_img" id="detail_img">
                    <div class="pic_view">
                        <a class="MagicZoomPlus" id="Zoomer" style="position: relative; display: inline-block; text-decoration: none; outline: 0px; overflow: hidden; width: auto; height: auto;">
                            <img :src="curShow.url" alt="" style="opacity: 1;">
                            ...
                            <div class="tempWrap" style="overflow:hidden; position:relative; width:330px">
                                <ul class="cle" style="width: 330px; position: relative; overflow: hidden; padding: 0px; margin: 0px; left: 0px;">
                                    <li v-for="(item, index) in proDetail.images" :class="{'current': index === curIndex}" @click="replaceShow(index, item)">
                                        <a>
                                          <img :src="item.url" alt="" @mouseover="showImageUrl(item)" @mouseout="hideImageUrl">
                                        </a>
                ...
                <div class="item-info" id="item-info">
                    <dl class="loaded">
                        ...
                        <p class="desc"> <span class="gray">{{proDetail.goodsBrief}}</span> </p>
                        </dt>
                        <dd class="property">
                            <ul>
                                <li v-if="proDetail.shipFree">此商品为免运费商品，计算配送金额时将不计入配送费用 </li>
                                <li>
                                    <span class="lbl">市场价</span> <em class="cancel">￥{{proDetail.shopPrice}}元</em></li>
                                <li>
                                    <span class="icon_promo">抢购</span> <span class="lbl">促销价</span><span class="unit"> <strong class="nala_price red" id="ECS_SHOPPRICE">￥{{proDetail.shopPrice}}元</strong> </span>  <span class="timedown" id="timedown"></span>
                                </li>
                                <li>
                                    <span class="lbl">销&nbsp;&nbsp;&nbsp;量</span><span>最近售出<em class="red">{{proDetail.soldNum}}</em>件</span></li>
                            </ul>
                        </dd>
                            ...
                            <ul class="sku">
                                <li class="skunum_li cle">
                                    ...
                                    <div class="skunum" id="skunum">
                                        ...
                                        <cite class="storage">(<font id="shows_number">{{proDetail.goodsNum}}件</font>)</cite>
                        ...
                        <div class="product_tabs">
                            <div class="sectionbox z-box" id="spxqitem">
                                <div class="spxq_main">
                                    ...
                                    <div class="spxq_dec">
                                      <ul class="cle">
                                        <li v-for="(item,index) in proDetail.descImages">
                                            <a>
...
import { getGoodsDetail, addShopCart,getShopCart,getCategorybyID } from '../../api/api';
  export default {
    data () {
        return {
            productId:{
    ...
    created () {
        this.productId.id = this.$route.params.productId;
    ...
    methods: {
        showImageUrl(url) {
      this.imageUrl = url;
      this.showImageUrlFlag = true;
    },
    hideImageUrl() {
      this.showImageUrlFlag = false;
    },
        getDetails () { //  请求商品详情
            console.log("logllllllllllllllllll")
          getGoodsDetail(this.productId).then((response)=> {
            this.proDetail = response.data.data;
            this.proDetail.images = JSON.parse(response.data.data.images).map(item=> {
            return {url: item}
          })
           this.proDetail.descImages = JSON.parse(response.data.data.descImages).map(item=> {
            return {url: item}
          })
            
                console.log("getGoods",response.data.data);
                console.log("getGoods",response.data.data.images);
    
                this.cate.id=response.data.data.categoryId
                getCategorybyID(this.cate).then((res)=> {
                    console.log("subCategory",res.data.data)
                     this.$set(this.proDetail,'category',res.data.data.name)
        ...
        addShoppingCart () { //加入购物车
            this.addShopCart.goods=Number(this.productId.id);
            this.addShopCart.nums=Number(this.buyNum);
            addShopCart(this.addShopCart).then((response)=> {
