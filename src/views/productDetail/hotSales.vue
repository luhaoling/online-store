        ...
        <ul>
            <li v-for="item in hotProduct">
              <router-link :to="'/app/home/productDetail/'+item.id" target=_blank>
                <img width="194px" height="194px" :src="item.goodsFrontImage">
                <p>{{item.goodsBrief}}</p>
                <p class="hot_price">￥{{item.shopPrice}}元</p>
              </router-link>
            </li>
        </ul>
    </div>
            ...
            getHotSales() { //请求热卖商品
              getHot()
                .then((response)=> {
                    ...
                    console.log("热卖商品",response.data.data.list)
                    this.hotProduct = response.data.data.list;
